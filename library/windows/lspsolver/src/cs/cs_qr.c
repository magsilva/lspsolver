...
csn *cs_qr (const cs *A, const css *S)
{
	...
	long vnz, rnz;
    int i, k, p, m, n, p1, top, m2, len, col, *s, *leftmost, *Ap, *Ai,
	*parent, *Rp, *Ri, *Vp, *Vi, *w, *pinv, *q ;
    ...
    vnz = (long)S->lnz ; rnz = (long)S->unz ; leftmost = S->leftmost ;
    w = (int*)cs_malloc (m2+n, sizeof (int)) ;	    /* get int workspace */
    x = (float*)cs_malloc (m2, sizeof (float)) ;	    /* get float workspace */
    ...
    N->B = Beta = (float*)cs_malloc (n, sizeof (float)) ;  /* allocate result Beta */
