...
int cs_ipvec (const int *p, const float *b, float *x, int n) ;
...
int cs_pvec (const int *p, const float *b, float *x, int n) ;
...
int cs_scatter (const cs *A, int j, float beta, int *w, float *x, int mark,
    cs *C, int nz) ;
csd *cs_scc (cs *A) ;
int cs_spsolve (cs *G, const cs *B, int k, int *xi, float *x,
